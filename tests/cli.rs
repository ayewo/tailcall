...
fn check_command_file_exists_but_is_invalid() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.arg("check").arg("tests/graphql/errors/test-const-with-inline.graphql");
    cmd.assert()
        .failure()
...
fn start_command_log_level() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .failure()
        .stderr(predicate::str::contains("error: unexpected argument '--log_level' found"));
    cmd.assert()
        .failure()
        .stderr(predicate::str::contains("Usage: tailcall start <FILE_PATH|--log-level <LOG_LEVEL>>"));
...
fn start_command_log_level2() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .failure()
        .stderr(predicate::str::contains("error: a value is required for '--log-level <LOG_LEVEL>' but none was supplied"));
    cmd.assert()
        .failure()
...
fn start_command_log_level3() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.assert()
        .failure()
        .stderr(predicate::str::contains("Usage: tailcall start --log-level <LOG_LEVEL> <FILE_PATH>"));
